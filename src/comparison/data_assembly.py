...
def generate_activity_btp_for_dc_schemes_and_cont(data_params, file_paths, save=False):
    ...
    for i in tqdm(range(len(dc_schemes))):
        ...
        if prev_cycle != cycle_length:
            ...
            btp_cont_column = get_continuous_btp_partitioned_for_dc_scheme(metric_col_name, location_df_above_SNR_and_detthresh.copy(),
        ...
        does_duty_cycled_df_have_less_dets_than_original(dc_applied_df, location_df_above_SNR_and_detthresh)
...
def generate_activity_call_rate_for_dc_schemes_and_cont(data_params, file_paths, save=False):
    ...
    for i in tqdm(range(len(dc_schemes))):
        ...
        if prev_cycle != cycle_length:
            prev_cycle = cycle_length
            callrate_cont_column = get_continuous_call_rates_partitioned_for_dc_scheme(metric_col_name, location_df_above_SNR_and_detthresh, data_params)
...
def get_continuous_call_rates_partitioned_for_dc_scheme(metric_col_name, location_df_above_SNR_and_detthresh, data_params):
    dc_tag_split = re.findall(r"\d+", metric_col_name)
    ...
    location_df = ss.assign_cycle_groups_to_each_call(location_df_above_SNR_and_detthresh, cycle_length, data_params)
...
def generate_activity_index_percent_for_dc_schemes_and_cont(data_params, file_paths, save=False):
    ...
    for i in tqdm(range(len(dc_schemes))):
        ...
        if prev_cycle != cycle_length:
            prev_cycle = cycle_length
            actvtind_cont_column = get_continuous_activity_index_partitioned_for_dc_scheme(metric_col_name, location_df_above_SNR_and_detthresh, data_params)
...
def get_continuous_activity_index_partitioned_for_dc_scheme(metric_col_name, location_df_above_SNR_and_detthresh, data_params):
    dc_tag_split = re.findall(r"\d+", metric_col_name)
    cycle_length = int(dc_tag_split[-1])
    cycle_length_in_secs = 60*cycle_length
...
def generate_activity_btp_for_false_positives_investigation(data_params, file_paths, save=False):
    ...
    dc_tag = '30of30'
    metric_col_name = f'{data_params["metric_tag"]} ({dc_tag})'
    btp_cont_column = get_continuous_btp_partitioned_for_dc_scheme(metric_col_name, location_df_above_SNR_and_detthresh.copy(), data_params, bout_params)
    ...
    for i in tqdm(range(len(fractions))):
        ...
        added_noise_location_df = location_df_above_SNR_and_detthresh.groupby(by='input_file_dt', group_keys=False).apply(lambda x : add_noise_to_group(x, frac))
        ...
        does_duty_cycled_df_have_less_dets_than_original(location_df_above_SNR_and_detthresh, dc_applied_df)
        bout_metrics = bt.generate_bout_metrics_for_location_and_freq(dc_applied_df, data_params, bout_params)
        bout_duration = actvt.get_bout_duration_per_cycle(bout_metrics, cycle_length_in_mins)
        bout_time_percentage = actvt.get_btp_per_time_on(bout_duration, time_on_in_secs)
...
def generate_activity_btp_for_false_negatives_investigation(data_params, file_paths, save=False):
    ...
    for i in tqdm(range(len(fractions))):
        ...
        removed_calls_location_df = location_df_above_SNR_and_detthresh.groupby(by='input_file_dt', group_keys=False).apply(lambda x : removed_calls_from_group(x, frac))
        dc_applied_df_reduced = ss.simulate_dutycycle_on_detections(removed_calls_location_df.copy(), data_params)
        does_duty_cycled_df_have_less_dets_than_original(dc_applied_df_reduced, location_df_above_SNR_and_detthresh)
...
def generate_activity_call_rate_for_false_positives_investigation(data_params, file_paths, save=False):
    ...
    dc_tag = '30of30'
    metric_col_name = f'{data_params["metric_tag"]} ({dc_tag})'
    callrate_arr = get_continuous_call_rates_partitioned_for_dc_scheme(metric_col_name, location_df_above_SNR_and_detthresh, data_params)
