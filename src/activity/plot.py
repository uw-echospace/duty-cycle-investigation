...
def plot_activity_grid_for_bouts(activity_arr, data_params, pipeline_params, file_paths):
    ...
    """
    activity_df = actvt.construct_activity_grid_for_bouts(activity_arr, data_params['cur_dc_tag'])
...
def plot_activity_grid_for_inds(activity_arr, data_params, pipeline_params, file_paths):
    ...
    """
    activity_df = actvt.construct_activity_grid_for_inds(activity_arr, data_params["cur_dc_tag"])
...
def plot_presence_grid(activity_arr, data_params, pipeline_params, file_paths):
    ...
    plt.axhline(y=np.where(activity_times==midnight_time)[0]-0.5, linewidth=0.5*len(activity_times), linestyle='dashed', color='k', label='Midnight 0:00 PST')
    plt.plot(np.arange(0, len(plot_dates)), ((sunrise_seconds_from_midnight / (30*60)) % len(plot_times)) - 0.5, 
...
def plot_dc_presence_comparisons_per_scheme(activity_arr, data_params, pipeline_params, file_paths):
    ...
    for i, dc_tag in enumerate(data_params['dc_tags']):
        ...
        plt.plot(np.arange(0, len(plot_dates)), ((sunset_seconds_from_midnight / (30*60)) % len(plot_times)) - 0.5, 
                color='k', linewidth=0.5*len(activity_times), linestyle='dashed', label=f'Time of Sunset')
        plt.axhline(y=np.where(activity_times==midnight_time)[0]-0.5, xmin=0, linewidth=0.5*len(activity_times), linestyle='dashed', color='k', label='Midnight 0:00 PST')
        plt.plot(np.arange(0, len(plot_dates)), ((sunrise_seconds_from_midnight / (30*60)) % len(plot_times)) - 0.5, 
                color='k', linewidth=0.5*len(activity_times), linestyle='dashed', label=f'Time of Sunrise')
        x, y = np.meshgrid(np.arange(presence_df.shape[1]), np.arange(presence_df.shape[0]))
        m = np.c_[x[presence_df == 1], y[presence_df == 1]]
...
def compare_metrics_per_night(activity_bouts_arr, activity_dets_arr, data_params, pipeline_params, file_paths):
    ...
    for i, date in enumerate(dates):
        ...
        activity_bouts_df = actvt.construct_activity_grid_for_bouts(activity_bouts_arr, dc_tag)
        on = int(dc_tag.split('of')[0])
        total = int(dc_tag.split('of')[1])
        recover_ratio = total / on
        ...
        activity_dets_df = actvt.construct_activity_grid_for_number_of_dets(activity_dets_arr, dc_tag)
        on = int(dc_tag.split('of')[0])
        total = int(dc_tag.split('of')[1])
        recover_ratio = total / on
...
def plot_numdets_n_activityinds(activity_dets_arr, activity_inds_arr, data_params, pipeline_params, file_paths):
    ...
    for date in dates:
        ...
        activity_inds_df = actvt.construct_activity_grid_for_inds(activity_inds_arr, dc_tag)
        on = int(dc_tag.split('of')[0])
        total = int(dc_tag.split('of')[1])
        recover_ratio = total / on
