...
def run_for_dets(data_params, pipeline_params, file_paths):
    ...
    if not(pipeline_params["read_csv"]):
        ...
        activity_arr = pd.read_csv(f'{file_paths["duty_cycled_folder"]}/{file_paths["dc_dets__TYPE_SITE_summary"]}.csv', index_col=0)
...
def run_for_bouts(data_params, pipeline_params, file_paths):
    ...
    if not(pipeline_params["read_csv"]):
        ...
        activity_arr = pd.read_csv(f'{file_paths["duty_cycled_folder"]}/{file_paths["dc_bouts__TYPE_SITE_summary"]}.csv', index_col=0)
...
def plot(activity_arr, data_params, pipeline_params, file_paths):
    ...
    plotting.plot_activity_grid(dh.construct_activity_grid_for_number_of_dets(activity_arr, data_params["cur_dc_tag"]), data_params, pipeline_params, file_paths)
