...
def check_machine_det_with_human_det(machine_det, human_det):
    ...
    return machine_detection_corresponds_to_human
...
def plot_spectrogram(ax, row, audio_seg, location_df_orig, test_df, fs, duration, start, osn_file_path, row_index, nfft):
    ...
    for _, det in plot_dets.iterrows():
        if (check_machine_det_with_human_det(det, row)):
    ...
    ax.text(
        x=int(fs * 0.001), y=0.80,
        s=f'Det {row_index+1} ({row["freq_group"]}) of {int(len(test_df))} added dets\nScore:{row["det_prob"]}', fontweight='bold', color='white', fontsize=10)
...
if __name__ == "__main__":
    ...
    test_df = location_df_kmeans[location_df_kmeans['det_prob']<0.5].reset_index()
